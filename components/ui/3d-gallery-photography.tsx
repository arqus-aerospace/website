...
function GalleryScene({
	...
	const upwardScrollAccum = useRef(0);
	const downwardScrollAccum = useRef(0);
	...
	const handleWheel = useCallback(
		(event: WheelEvent) => {
			...
			if (event.deltaY < 0) {
				upwardScrollAccum.current += Math.abs(event.deltaY);
				downwardScrollAccum.current = 0;
				if (upwardScrollAccum.current > 600 && onEscapeTop) {
					upwardScrollAccum.current = 0;
					onEscapeTop();
					return;
				}
			} else {
				downwardScrollAccum.current += event.deltaY;
				upwardScrollAccum.current = 0;
				if (downwardScrollAccum.current > 600 && onEscapeBottom) {
					downwardScrollAccum.current = 0;
					onEscapeBottom();
					return;
				}
		...
		[speed, onEscapeTop, onEscapeBottom]
	);
	...
	return (
		<>
			{planesData.current.map((plane, i) => {
				...
				const label = (normalizedImages[plane.imageIndex] as { label?: string })?.label;
				return (
					<ImagePlane
						key={plane.index}
						texture={texture}
