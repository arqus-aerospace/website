...
export default function Arena({
  ...
  const modeRef = useRef<"manual" | "auto">(forceMode ?? "manual");
  const autoTimerRef = useRef<number | null>(null);

  const [score, setScore] = useState(0);
  const [mode, setMode] = useState<"manual" | "auto">(forceMode ?? "manual");
  ...
  useEffect(() => {
    ...
    function destroyDrone(dr: Drone) {
      ...
      window.setTimeout(() => {
        ...
        if (variant === "interactive") setScore((s) => s + 1);
        window.setTimeout(() => {
    ...
    const tick = (now: number) => {
      ...
      for (const dr of dronesRef.current) {
        ...
        dr.x += dr.vx * dt;
        dr.y += dr.vy * dt;
        if (dr.x <= 0) { dr.x = 0; dr.vx = Math.abs(dr.vx); }
        if (dr.x + dr.w >= aw) { dr.x = aw - dr.w; dr.vx = -Math.abs(dr.vx); }
        if (dr.y <= top) { dr.y = top; dr.vy = Math.abs(dr.vy); }
        if (dr.y >= bottom) { dr.y = bottom; dr.vy = -Math.abs(dr.vy); }
        paintDrone(dr);
      }
    ...
    const onMove = (e: MouseEvent) => {
      if (modeRef.current !== "manual" || variant !== "interactive") return;
    ...
    if (variant === "interactive") arena.addEventListener("mousemove", onMove);
  ...
  const className =
    variant === "background"
      ? "thesis-arena arena--background auto-mode"
      : "thesis-arena";
  ...
  return (
    <div
      ...
      aria-label={
        variant === "interactive"
          ? "Click drones to destroy them with the Arqus laser tower"
          : undefined
      }
    >
      ...
      {variant === "interactive" && (
        <>
          ...
          <div className="arena-modes" role="group" aria-label="Engagement mode">
            <button
        ...
        <svg viewBox="0 0 80 160" preserveAspectRatio="xMidYEnd meet">
          ...
          <g ref={turretRef} className="turret">
            ...
            <rect
              ...
              fill="#3a4150"
            />
            <circle cx="40" cy="34" r="2.4" fill="#E62630" />
          </g>
        </svg>
      </div>
    </div>
